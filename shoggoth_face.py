...
class ShoggothFace:
    ...
    def __init__(
        self,
        shoggoth_model_name: str | None = None,
        face_model_name: str | None = None,
        *,
        # Preloaded components (optional). If supplied, models will NOT be reloaded.
        shoggoth_model=None,
        face_model=None,
        tokenizer=None,
        device: str = None,
        ...
        # ------------------------------------------------------------------
        # Load or accept pre-initialized models & tokenizer
        # ------------------------------------------------------------------
        if face_model is not None and shoggoth_model is not None and tokenizer is not None:
            # Use supplied components (no loading)
            self.face_model = face_model
            self.shoggoth_model = shoggoth_model
            self.tokenizer = tokenizer
        else:
            # Fall back to loading from repo names (original behaviour)
            if shoggoth_model_name is None or face_model_name is None:
                raise ValueError("When preloaded models are not provided, 'shoggoth_model_name' and 'face_model_name' must be specified.")
    ...
    def generate(self, prompt_inputs, max_thinking_tokens, max_new_tokens, **gen_kwargs):
        ...
        # Reset stateful logits processors so each turn starts fresh
